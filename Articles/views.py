...
def SignHandle(request):
    if request.method=='POST':
        ...
        try:
            if (otp == OTP):
                if Username.isalnum():
                    user=CustomUser.objects.create_user(username=Username,first_name=First,email=Email,password=password,Phone_Number=Number,Address=address,profile_pic=Pic)
                    user.save()
                    login(request,user)
...
def HandlePost(request,Slug):
    if request.method == "POST":
        ...
        if request.user.is_authenticated:
            Comment = Comments(Author=request.user.username,Content=comment,Slug=Slug,author_pic=request.user.profile_pic)
        else:
    ...
    return render(request,'articles.html',{"post":post,"comment":comment,"related_post":related_post,"no_of_comment":comment.count()})
